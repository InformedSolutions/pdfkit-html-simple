...
exports.transformHTML = function (html) {
  ...
  // If an <a> is found within a <p> before it closes, it breaks
  // just make it regular text for now.
  $('a').each(function() {
    const text = this.children[0].data;
    
    if (this.parent.tagName === 'p') {
...
exports.convertNodeToTransformations = function (node, options) {
  ...
  if (node.attributes.find(a => a.name === 'href')) {
    const link = node.attributes.filter(a => a.name === 'href');
    textOptions.link = link[0].value;
  }
  ...
  transformations.push(function (document) {
    ...
    if (node.tagName === 'li') {
      const itemText = cleanText(node.text);
      // how deep is the list based on the number of 'ul' elements that parent it
      const depth = node.tagNames.filter(x => x === 'ul').length;

      let listItem = [itemText];
      for (let i = 0; i < depth; i++) {
        listItem = [listItem]
      }
      document.list(listItem, { textIndent: (20), bulletIndent: 20 });
    } else {
      // Set to blue if hyperlinked text
      if (textOptions.link) {
        document.fillColor('blue');
      }

      document.text(node.text ? cleanText(node.text) + ' ' : '', textOptions)
        .fillColor('black');
